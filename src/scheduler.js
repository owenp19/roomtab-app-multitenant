    ...
    try {
      ...
      for (var i = 0; i < rows.length; i++) {
        ...
        if (diffDays <= 7 && diffDays >= 0) {
          var [existing] = await pool.query(
            "SELECT id FROM notifications WHERE product_name = ? AND room_id = ? AND expiration_date = ? AND is_read = 0 AND tenant_id = ? LIMIT 1",
          ...
          if (!existing || existing.length === 0) {
            await pool.query(
              ...
              [item.product_name, item.room_id, item.floor_id, item.room_number, item.floor_name, item.expiration_date, diffDays, "Producto " + item.product_name + " vence en " + diffDays + " día(s) en habitación " + item.room_number, item.tenant_id]
            );
            created.expiring++;
          }
        }
      }
      if (created.expiring > 0) console.log("[Scheduler] Created " + created.expiring + " expiration notification(s)");
